...
export const LedgerService = {
    ...
    async recomputeDailyStats(date: string): Promise<void> {
        ...
        for (const log of logs) {
            ...
            const weight = log.weight; // integer -10 to 10 (delta) or sum of deltas

            // Activity = (Credit * Delta) / 10
            const valA = (credit * weight) / 10.0;
        ...
        for (const t of completedTasks) {
            // "Earned is the sum of the credits of tasks whose progress is 1.0" -> now 10
            if (t.progress >= 10) { // Integer 10 = 100%
