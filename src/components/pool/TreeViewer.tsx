...
const NodeItem: React.FC<NodeItemProps> = ({
    node, level, childNodes, allNodes, onSelect, selectedId, expandedIds, toggleExpand,
    creatingParentId, onConfirmCreate, onCancelCreate
}) => {
            ...
            {isExpanded && (
                <>
                    {childNodes.map(child => {
                        const grandkids = allNodes.filter(n => n.parentId === child.id).sort((a, b) => a.order - b.order);
                        return (
                            <NodeItem
                                key={child.id}
                                node={child}
...
const EditBox: React.FC<{ onSave: (val: string) => void, onCancel: () => void }> = ({ onSave, onCancel }) => {
...
export const TreeViewer: React.FC = () => {
    ...
    const handleConfirmCreate = async (name: string) => {
        try {
            ...
            // Auto-generate code
            let code = name.toUpperCase().replace(/[^A-Z0-9]/g, '').substring(0, 6);
    ...
    return (
        <div style={{ display: 'flex', gap: '16px', height: '100%', flexDirection: 'row' }}>
            <Panel title="Node Tree" className="tree-panel" actions={<Button onClick={handleStartCreateRoot}>+ Root</Button>}>
                <div className="tree-content">
                    {!nodes || nodes.length === 0 ? (
                        creatingParentId !== 'ROOT' && <div style={{ padding: 16, color: 'var(--text-secondary)' }}>No nodes. Create a root.</div>
                    ) : (
                        rootNodes.map(node => (
                            <NodeItem
                                key={node.id}
                                node={node}
                                level={0}
                                childNodes={nodes.filter(n => n.parentId === node.id).sort((a, b) => a.order - b.order)}
                                allNodes={nodes}
            ...
            <div style={{ flex: 1, minWidth: 300 }}>
                {selectedNode ? (
                    <Panel
                        ...
                        actions={
                            <div style={{ display: 'flex', gap: 8 }}>
                                <Button onClick={handleStartCreateChild} style={{ fontSize: '0.8em' }}>+ Child</Button>
                                <Button onClick={handleDeleteNode} style={{ fontSize: '0.8em', borderColor: '#d32f2f', color: '#d32f2f' }}>Delete</Button>
                            </div>
                        }
                    >
                        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                            <div style={{ marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                ...
                                <Input
                                    ...
                                    onBlur={handleUpdateDetails}
                                />
                                <div className="text-dim" style={{ fontSize: '0.8em' }}>{selectedNode.id}</div>
                            </div>
                    ...
                    <Panel title="Details">
                        <div style={{ padding: 16, color: 'var(--text-secondary)' }}>Select a node to view tasks.</div>
                    </Panel>
                )}
            </div>
        </div>
