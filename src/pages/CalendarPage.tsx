...
import { startOfMonth, endOfMonth, eachDayOfInterval, format, addMonths, subMonths, isToday } from 'date-fns';
...
export const CalendarPage: React.FC = () => {
    ...
    return (
        <div style={{ padding: '16px', height: '100%', overflow: 'auto', maxWidth: '800px', margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <Button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}>&lt; Prev</Button>
                <div style={{ fontSize: '1.2em', fontWeight: 'bold' }}>{format(currentMonth, 'MMMM yyyy')}</div>
            ...
            <Panel title="Calendar">
                <div style={{
                    ...
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                        <div key={d} style={{ background: 'var(--panel-bg)', padding: 8, textAlign: 'center', fontWeight: 'bold' }}>{d}</div>
                    ))}
                    ...
                    {days.map(day => {
                        ...
                        return (
                            ...
                            >
                                <div style={{ textAlign: 'right', fontWeight: 'bold', marginBottom: 4 }}>{format(day, 'd')}</div>
                                {stat && (
                                    <div style={{ fontSize: '0.8em', display: 'flex', flexDirection: 'column', gap: 2 }}>
